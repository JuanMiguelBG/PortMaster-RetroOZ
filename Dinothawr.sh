...
PORT_LOG_FILE="/roms/logs/dinothawr.log"

echo "Dinothawr: " | tee $PORT_LOG_FILE
...
echo "Launch command: $raloc/retroarch $raconf -L $GAMEDIR/dinothawr_libretro.so  $GAMEDIR/dinothawr.game 2>&1 | tee -a $PORT_LOG_FILE" | tee -a $PORT_LOG_FILE
$raloc/retroarch $raconf -L $GAMEDIR/dinothawr_libretro.so  $GAMEDIR/dinothawr.game 2>&1 | tee -a $PORT_LOG_FILE
